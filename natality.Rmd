...
Xnames <- c(Xnames, "drink", "wtgain")
Xnames <- setdiff(Xnames, c("pldel", "birattnd"))
...
```{r}
tau_bin.forest <- causal_forest(Xmod, Ymod, Wmod_bin)
```

```{r}
tau_cnt.forest <- causal_forest(Xmod, Ymod, Wmod_cnt)
```
...
```{r}
tau_bin.hat <- predict(tau_bin.forest)$predictions
hist(tau_bin.hat)
```
```{r}
tau_cnt.hat <- predict(tau_cnt.forest)$predictions
hist(tau_cnt.hat)
```
```{r}
...
```{r}
tau_bin.blp <- best_linear_projection(tau_bin.forest, Xmod)
tau_bin.blp
```
