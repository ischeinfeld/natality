...
Births are filtered to those in hospitals with a medical doctor present. A new 
variable `smoking` is created with three levels, `none, light, heavy`. Three
...
if (preprocess) {
  ...
  natl_full <- read_rds(subset_fname) %>%
    ...
    # filter to hospital birth with doctor attending
    filter(pldel == 1 && birattnd == 1) %>%
